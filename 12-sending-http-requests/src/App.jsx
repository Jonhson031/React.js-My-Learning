...
import { updateUserPlaces, fetchData } from './fetchData.js';
...
function App() {
  ...
  useEffect(() => {
    async function getUserPlaces() {
      try {
        const data = await fetchData('http://localhost:3000/user-places');
        setUserPlaces(data);
      } catch (err) {
        console.error(err);
        setError({ message: err.message || 'Could not load user places, please try again later!' });
      } finally {
        setIsLoading(false);
      }
    }
  ...
  const handleRemovePlace = useCallback(
    async function handleRemovePlace() {
      ...
      try {
        ...
        updateUserPlaces(userPlaces);
        setErrorUpdatingPlaces({ message: err.message || 'Failed to delete places' });
      }
